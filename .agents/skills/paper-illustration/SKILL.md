...
- **Never pass `— style-ref` (or the cache contents) to the Codex vision-checker / Gemini reasoning-checker sub-agents** when they score the generated image — the image must be judged on its own merits.
