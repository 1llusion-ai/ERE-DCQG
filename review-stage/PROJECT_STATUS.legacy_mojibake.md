...
| answer phrase 瑙勫垯杩囧 | prefilter pass rate 杩囬珮涓斾細鎶婃埅鏂?phrase 鏍囨垚 valid | **已修复** — added dangling-word/bracket/passive completeness checks |
...
## 18. Current Research Status: Valid Yield vs Difficulty Controllability (2026-05-03)

This section records the current central issue. Future debugging or experiment planning must start from these findings, not from the earlier assumption that hop count alone proves difficulty control.

### 18.1 What Has Been Demonstrated

The current new-framework pipeline has demonstrated a clear **valid path-grounded question yield** advantage for PathQG-HardAware.

Baseline alignment pilot:

- Run directory: `outputs/runs/baseline_alignment_pilot/`
- Input: same 76 strict paths for all methods
- Filter: same v3 quality filter for all methods
- Methods:
  - `PathQG-HardAware`
  - `ZeroShot-TargetQG`
  - `ICL-TargetQG`
...
- PathQG-HardAware is much better at producing questions that satisfy the current event-path validation pipeline.
- PathQG-HardAware is the only method that produced filter-passing Hard candidates on this strict-path pilot.
- This supports a claim about **event-path-grounded controllability / valid constrained question generation**.

Important limitation:

- This does **not** by itself prove that the generated questions are independently judged as Easy / Medium / Hard according to the target difficulty.

### 18.2 Independent Difficulty Evaluation Finding

Independent difficulty evaluation was added to avoid using the internal filter as the difficulty metric.
...
- Difficulty-only judge does not see target difficulty.
- Difficulty-only judge does not use solver answer or solver correctness.
- Path-dependency judge separately evaluates whether the generated question depends on the event path.
...
- However, independent judge does not consider current PathQG Hard questions genuinely Hard.
- Current Hard questions are usually judged Easy or Medium.
- Therefore, the current evidence supports **path dependency**, but not yet **difficulty controllability**.

### 18.3 Why Hard Is Not Yet Solved

Current Hard generation often produces questions of the form:

```text
After event A and event B, what happened?
```

This is path-dependent, but it explicitly exposes much of the reasoning chain in the question. The independent judge often treats such questions as Medium or Easy because the answer can be located after the question tells the reader which events to connect.

Root issue:

- Hop count and path dependency are not the same as cognitive difficulty.
- A 3-hop / 4-event path can still produce a Medium-level question if the question gives away the chain.
- Many MAVEN-ERE paths are short and their final event is often recoverable from a single answer sentence.

Current safe statement:

```text
PathQG-HardAware improves valid path-grounded question yield and path dependency.
```

Do not claim yet:

```text
PathQG-HardAware generates more difficulty-consistent Hard questions.
```

### 18.4 Implicit Hard Prompt Experiment

An implicit-chain Hard prompt was tested to avoid explicitly listing the full path in the question.
...
- The implicit-chain idea preserves path dependency among surviving examples.
- But it lowers filter pass rate and still does not reliably produce independently judged Hard questions.
- The current implicit Hard prompt should remain experimental and should not be merged as the default Hard prompt.

### 18.5 Current Main Bottlenecks

1. **Difficulty controllability is not yet independently supported.**
   - Valid yield is strong.
   - Path dependency is improved.
   - Hard difficulty consistency is not yet demonstrated.

2. **Hard strict pool is too small.**
   - Current strict-path pilot has only 9 Hard paths.
   - This is insufficient for final claims.

3. **Hard generation strategy is still weak.**
   - Explicit Hard prompt creates Medium-like questions.
   - Implicit Hard prompt reduces pass rate and still does not yield Pred Hard reliably.

4. **Answer phrase quality still affects evaluation.**
   - Some generated/evaluated answers are awkward or truncated in semantic terms even if they pass basic extraction checks.
   - Examples observed in prior audits include incomplete phrases such as short fragments or semantically weak answer phrases.

5. **Solver/judge correctness remains auxiliary only.**
   - Solver accuracy is currently low and judge calibration is not complete.
   - Solver correctness must not be used as the primary difficulty-control metric yet.

### 18.6 Recommended Next Direction

Do not keep tuning the single Hard prompt blindly on the same 9 paths.

Recommended next experiments:

1. **Multi-candidate Hard generation + independent difficulty selection**
   - For each Hard path, generate K candidates, e.g. K=5.
   - Run quality filter.
   - Run independent difficulty judge.
   - Select candidates predicted as Hard, or best Medium if no Hard exists.
   - Report valid yield, Pred Hard yield, and path dependency.

2. **Increase Hard path potential**
   - Sample longer paths, e.g. 5+ events if available.
   - Prefer paths with CAUSE / SUBEVENT / MIXED relation composition.
   - Prefer paths spanning more supporting sentences.
   - Avoid paths where final answer is a direct single-sentence lookup.

3. **Keep two separate claims**
   - Claim A: valid path-grounded generation yield.
   - Claim B: independent difficulty consistency.
   - Claim A is currently supported.
   - Claim B still needs further evidence.

4. **Human calibration**
   - Manually label a small set of generated questions for:
     - answerability
     - final-event consistency
     - path dependency
     - required inference steps
     - human difficulty label
   - Use this to calibrate or challenge GPT-4o-mini difficulty judge.

### 18.7 Rules For Future Work

- Do not use internal filter pass as difficulty accuracy.
- Do not use target difficulty in the difficulty-only judge prompt.
- Do not use solver answer or solver correctness as input to the independent difficulty judge.
- Do not reintroduce four-dimensional difficulty scoring.
- Do not depend on `event_qg/`; use the `dcqg/` package and root `scripts/`.
- Always inspect trace / JSONL examples before explaining a failure mode.
- When reporting baseline results, separate:
