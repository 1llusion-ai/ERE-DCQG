...
1. **Logging must be active.** Copy `templates/Codex-hooks/meta_logging.json` into your project's `.Codex/settings.json` (or merge the hooks section).
...
if [ ! -f "$EVENTS_FILE" ]; then
    ...
    echo "Enable logging first: copy templates/Codex-hooks/meta_logging.json into .Codex/settings.json"
    exit 1
fi
...
1. **Passive logging** (always on): Codex hooks record events to `.aris/meta/events.jsonl` automatically during normal usage. Zero user effort.
