...
2. **Google Gemini CLI** — installed and authenticated
   ...
   gemini auth
   ```
3. **gemini-mcp-tool** — MCP bridge for Codex ([jamubc/gemini-mcp-tool](https://github.com/jamubc/gemini-mcp-tool))
   ```bash
...
In `~/.Codex.json` (or `%APPDATA%\Codex\claude_desktop_config.json` for Codex Desktop), add:
...
Gemini CLI uses your Google account or an API key. Add to `.Codex/.env`:
...
Codex automatically loads `.Codex/.env` as environment variables.
