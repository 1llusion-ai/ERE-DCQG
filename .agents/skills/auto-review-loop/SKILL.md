...
- **REVIEWER_DIFFICULTY = medium** — Controls how adversarial the reviewer is. Three levels:
  - `medium` (default): Current behavior — MCP-based review, Codex controls what context GPT sees.
  - `hard`: Adds **Reviewer Memory** (GPT tracks its own suspicions across rounds) + **Debate Protocol** (Codex can rebut, GPT rules).
  - `nightmare`: Everything in `hard` + **GPT reads the repo directly** via `codex exec` (Codex cannot filter what GPT sees) + **Adversarial Verification** (GPT independently checks if code matches claims).
...
mcp__codex__codex:
  ...
  prompt: |
    ...
    The author (Codex) controls what context you see — be skeptical
...
## Instructions
You have FULL READ ACCESS to this repository. The author (Codex) does NOT
...
**Key difference**: In nightmare mode, GPT independently reads code, result files, and logs. Codex cannot filter or curate what GPT sees. This is the closest analog to a real hostile reviewer who reads your actual paper + supplementary materials.
...
After parsing the review, Codex (the author) gets a chance to **rebut**:

**Step 1 — Codex's Rebuttal:**

For each weakness the reviewer identified, Codex writes a structured response:
...
Send Codex's rebuttal back to GPT for a ruling:
